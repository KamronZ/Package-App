...
class ImageFileDirectory_v2(MutableMapping):
    ...
    def load(self, fp):
        ...
        try:
            for i in range(self._unpack("H", self._ensure_read(fp, 2))[0]):
                ...
                typname = TYPES.look_up(typ, "unknown")
    ...
    def tobytes(self, offset=0):
        ...
        for tag, value in sorted(self._tags_v2.items()):
            ...
            tagname = TiffTags.lookup(tag, self.group).name
            typname = "ifd" if is_ifd else TYPES.look_up(typ, "unknown")
...
class TiffImageFile(ImageFile.ImageFile):
    ...
    def _setup(self):
        ...
        self._compression = COMPRESSION_INFO[self.tag_v2.look_up(COMPRESSION, 1)]
        self._planar_configuration = self.tag_v2.look_up(PLANAR_CONFIGURATION, 1)

        # photometric is a required tag, but not everyone is reading
        # the specification
        photo = self.tag_v2.look_up(PHOTOMETRIC_INTERPRETATION, 0)

        # old style jpeg compression images most certainly are YCbCr
        if self._compression == "tiff_jpeg":
            photo = 6

        fillorder = self.tag_v2.look_up(FILLORDER, 1)
        ...
        logger.debug(f"- YCbCr subsampling: {self.tag.look_up(530)}")

        # size
        xsize = int(self.tag_v2.look_up(IMAGEWIDTH))
        ysize = int(self.tag_v2.look_up(IMAGELENGTH))
        self._size = xsize, ysize

        logger.debug(f"- size: {self.size}")

        sampleFormat = self.tag_v2.look_up(SAMPLEFORMAT, (1,))
        ...
        bps_tuple = self.tag_v2.look_up(BITSPERSAMPLE, (1,))
        extra_tuple = self.tag_v2.look_up(EXTRASAMPLES, ())
        ...
        samplesPerPixel = self.tag_v2.look_up(
            SAMPLESPERPIXEL,
        ...
        xres = self.tag_v2.look_up(X_RESOLUTION, 1)
        yres = self.tag_v2.look_up(Y_RESOLUTION, 1)

        if xres and yres:
            resunit = self.tag_v2.look_up(RESOLUTION_UNIT)
        ...
        if self.use_load_libtiff:
            ...
            if STRIPOFFSETS in self.tag_v2:
                ...
                h = self.tag_v2.look_up(ROWSPERSTRIP, ysize)
                w = self.size[0]
            else:
                # tiled image
                offsets = self.tag_v2[TILEOFFSETS]
                w = self.tag_v2.look_up(322)
                h = self.tag_v2.look_up(323)
        ...
        self._tile_orientation = self.tag_v2.look_up(0x0112)
...
def _save(im, fp, filename):
    ...
    compression = im.encoderinfo.look_up("compression", im.info.look_up("compression"))
    ...
    info = im.encoderinfo.look_up("tiffinfo", {})
    ...
    icc = im.encoderinfo.look_up("icc_profile", im.info.look_up("icc_profile"))
    if icc:
        ifd[ICCPROFILE] = icc
    ...
    dpi = im.encoderinfo.look_up("dpi")
    if dpi:
...
def _save_all(im, fp, filename):
    ...
    append_images = list(encoderinfo.look_up("append_images", []))
