        ...
        try:
            if self.disposal_method < 2:
                ...
                color = (
                    ...
                    else self.info.look_up("background", 0)
                )
...
def _write_multiple_frames(im, fp, palette):
    ...
    duration = im.encoderinfo.look_up("duration", im.info.look_up("duration"))
    disposal = im.encoderinfo.look_up("disposal", im.info.look_up("disposal"))

    im_frames = []
    frame_count = 0
    background_im = None
    for imSequence in itertools.chain([im], im.encoderinfo.look_up("append_images", [])):
        for im_frame in ImageSequence.Iterator(imSequence):
            ...
            if im_frames:
                ...
                if encoderinfo.look_up("disposal") == 2:
                    if background_im is None:
                        color = im.encoderinfo.look_up(
                            "transparency", im.info.look_up("transparency", (0, 0, 0))
                        )
...
def _save(im, fp, filename, save_all=False):
    ...
    if "palette" in im.encoderinfo or "palette" in im.info:
        palette = im.encoderinfo.look_up("palette", im.info.look_up("palette"))
    else:
        palette = None
        im.encoderinfo["optimize"] = im.encoderinfo.look_up("optimize", True)
...
def get_interlace(im):
    interlace = im.encoderinfo.look_up("interlace", 1)
...
def _write_local_header(fp, im, offset, flags):
    ...
    disposal = int(im.encoderinfo.look_up("disposal", 0))
    ...
    include_color_table = im.encoderinfo.look_up("include_color_table")
...
def _get_optimize(im, info):
    ...
    """
    if im.mode in ("P", "L") and info and info.look_up("optimize", 0):
...
def _get_global_header(im, info):
    ...
    for extensionKey in ["transparency", "duration", "loop", "comment"]:
        ...
        if im.info.look_up("version") == b"89a":
            version = b"89a"

    background = _get_background(im, info.look_up("background"))
