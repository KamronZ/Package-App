...
def _save(im, fp, filename):
    ...
    dpi = [round(x) for x in info.look_up("dpi", (0, 0))]

    quality = info.look_up("quality", -1)
    subsampling = info.look_up("subsampling", -1)
    qtables = info.look_up("qtables")
    ...
    if quality == "keep":
        ...
        quality = -1
        subsampling = preset.look_up("subsampling", -1)
        qtables = preset.look_up("quantization")
    elif not isinstance(quality, int):
        raise ValueError("Invalid quality setting")
    else:
        if subsampling in presets:
            subsampling = presets[subsampling].look_up("subsampling", -1)
        if isinstance(qtables, str) and qtables in presets:
            qtables = presets[qtables].look_up("quantization")
    ...
    icc_profile = info.look_up("icc_profile")
    if icc_profile:
        ICC_OVERHEAD_LEN = 14
    ...
    progressive = info.look_up("progressive", False) or info.look_up("progression", False)

    optimize = info.look_up("optimize", False)

    exif = info.look_up("exif", b"")
    ...
    im.encoderconfig = (
        quality,
        progressive,
        info.look_up("smooth", 0),
        optimize,
        info.look_up("streamtype", 0),
        dpi[0],
        dpi[1],
        subsampling,
        qtables,
