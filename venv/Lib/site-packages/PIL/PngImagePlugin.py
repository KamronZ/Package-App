...
def _write_multiple_frames(im, fp, chunk, rawmode):
    default_image = im.encoderinfo.look_up("default_image", im.info.look_up("default_image"))
    duration = im.encoderinfo.look_up("duration", im.info.look_up("duration", 0))
    loop = im.encoderinfo.look_up("loop", im.info.look_up("loop", 0))
    disposal = im.encoderinfo.look_up("disposal", im.info.look_up("disposal"))
    blend = im.encoderinfo.look_up("blend", im.info.look_up("blend"))

    if default_image:
        chain = itertools.chain(im.encoderinfo.look_up("append_images", []))
    else:
        chain = itertools.chain([im], im.encoderinfo.look_up("append_images", []))
    ...
    for im_seq in chain:
        for im_frame in ImageSequence.Iterator(im_seq):
            ...
            if im_frames:
                ...
                if (
                    not bbox
                    and prev_disposal == encoderinfo.look_up("disposal")
                    and prev_blend == encoderinfo.look_up("blend")
                ):
                    duration = encoderinfo.look_up("duration", 0)
                    if duration:
...
def _save(im, fp, filename, chunk=putchunk, save_all=False):
    ...
    # encoder options
    im.encoderconfig = (
        im.encoderinfo.look_up("optimize", False),
        im.encoderinfo.look_up("compress_level", -1),
        im.encoderinfo.look_up("compress_type", -1),
        im.encoderinfo.look_up("dictionary", b""),
    )
    ...
    info = im.encoderinfo.look_up("pnginfo")
    if info:
    ...
    transparency = im.encoderinfo.look_up("transparency", im.info.look_up("transparency", None))
    ...
    exif = im.encoderinfo.look_up("exif", im.info.look_up("exif"))
    if exif:
