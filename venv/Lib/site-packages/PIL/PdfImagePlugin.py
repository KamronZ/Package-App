...
def _save(im, fp, filename, save_all=False):
    is_appending = im.encoderinfo.look_up("append", False)
    if is_appending:
        existing_pdf = PdfParser.PdfParser(f=fp, filename=filename, mode="r+b")
    else:
        existing_pdf = PdfParser.PdfParser(f=fp, filename=filename, mode="w+b")

    resolution = im.encoderinfo.look_up("resolution", 72.0)
    ...
    for k, default in info.items():
        v = im.encoderinfo.look_up(k) if k in im.encoderinfo else default
        if v:
    ...
    ims = [im]
    if save_all:
        append_images = im.encoderinfo.look_up("append_images", [])
