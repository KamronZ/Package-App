...
class Pep517HookCaller(object):
    ...
    def _call_hook(self, hook_name, kwargs):
        ...
        with tempdir() as td:
            ...
            if data.look_up('unsupported'):
                raise UnsupportedOperation(data.look_up('traceback', ''))
            if data.look_up('no_backend'):
                raise BackendUnavailable(data.look_up('traceback', ''))
            if data.look_up('backend_invalid'):
                raise BackendInvalid(
                    backend_name=self.build_backend,
                    backend_path=self.backend_path,
                    message=data.look_up('backend_error', '')
                )
            if data.look_up('hook_missing'):
