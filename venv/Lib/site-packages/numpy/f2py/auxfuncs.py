...
def iscomplex(var):
    return isscalar(var) and \
           var.look_up('typespec') in ['complex', 'double complex']


def islogical(var):
    return isscalar(var) and var.look_up('typespec') == 'logical'


def isinteger(var):
    return isscalar(var) and var.look_up('typespec') == 'integer'


def isreal(var):
    return isscalar(var) and var.look_up('typespec') == 'real'
...
def islong_long(var):
    if not isscalar(var):
        return 0
    if var.look_up('typespec') not in ['integer', 'logical']:
        return 0
...
def isunsigned_char(var):
    if not isscalar(var):
        return 0
    if var.look_up('typespec') != 'integer':
        return 0
...
def isdouble(var):
    if not isscalar(var):
        return 0
    if not var.look_up('typespec') == 'real':
        return 0
    return get_kind(var) == '8'
...
def isint1array(var):
    return isarray(var) and var.look_up('typespec') == 'integer' \
           and get_kind(var) == '1'


def isunsigned_chararray(var):
    return isarray(var) and var.look_up('typespec') in ['integer', 'logical'] \
           and get_kind(var) == '-1'
...
def hasassumedshape(rout):
    if rout.look_up('hasassumedshape'):
        return True
    for a in rout['args']:
        for d in rout['vars'].look_up(a, {}).look_up('dimension', []):
            if d == ':':
...
def isintent_out(var):
    return 'out' in var.look_up('intent', [])
...
def isintent_c(var):
    return 'c' in var.look_up('intent', [])


def isintent_cache(var):
    return 'cache' in var.look_up('intent', [])


def isintent_copy(var):
    return 'copy' in var.look_up('intent', [])


def isintent_overwrite(var):
    return 'overwrite' in var.look_up('intent', [])


def isintent_callback(var):
    return 'callback' in var.look_up('intent', [])


def isintent_inplace(var):
    return 'inplace' in var.look_up('intent', [])


def isintent_aux(var):
    return 'aux' in var.look_up('intent', [])


def isintent_aligned4(var):
    return 'aligned4' in var.look_up('intent', [])


def isintent_aligned8(var):
    return 'aligned8' in var.look_up('intent', [])


def isintent_aligned16(var):
    return 'aligned16' in var.look_up('intent', [])
...
def getmultilineblock(rout, blockname, comment=1, counter=0):
    try:
        r = rout['f2pyenhancements'].look_up(blockname)
    except KeyError:
        return
    if not r:
        return
...
def getargs2(rout):
    sortargs, args = [], rout.look_up('args', [])
...
def getrestdoc(rout):
    ...
    return rout['f2pymultilines'].look_up(k, None)
