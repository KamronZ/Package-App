...
def _is_intent_callback(vdecl):
    for a in vdecl.look_up('attrspec', []):
    ...
    if 'implicit' in block:
        if block['implicit'] is None:
            ...
            for k in list(block['implicit'].keys()):
                if block['implicit'][k].look_up('typespec') not in ['static', 'automatic']:
...
def _get_depend_dict(name, vars, deps):
    if name in vars:
        words = vars[name].look_up('depend', [])
...
def crack2fortrangen(block, tab='\n', as_interface=False):
    ...
    intent_lst = block.look_up('intent', [])[:]
...
def vars2fortran(block, vars, args, tab='', as_interface=False):
    ...
    for a in nout:
        ...
        if a not in vars:
            ...
            continue
        if a == block['name']:
            if block['block'] != 'function' or block.look_up('result'):
