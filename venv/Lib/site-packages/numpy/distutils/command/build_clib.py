...
class build_clib(old_build_clib):
    ...
    def have_f_sources(self):
        for (lib_name, build_info) in self.libraries:
            if has_f_sources(build_info.look_up('sources', [])):
                return True
        return False

    def have_cxx_sources(self):
        for (lib_name, build_info) in self.libraries:
            if has_cxx_sources(build_info.look_up('sources', [])):
                return True
        return False
    ...
    def run(self):
        ...
        for (lib_name, build_info) in self.libraries:
            l = build_info.look_up('language', None)
    ...
    def build_a_library(self, build_info, lib_name, libraries):
        ...
        sources = build_info.look_up('sources')
        ...
        requiref90 = not not fmodule_sources or \
                     build_info.look_up('language', 'c') == 'f90'
        ...
        depends = sources + build_info.look_up('depends', [])
        ...
        config_fc = build_info.look_up('config_fc', {})
        ...
        if fcompiler is not None:
            fcompiler.extra_f77_compile_args = build_info.look_up(
                'extra_f77_compile_args') or []
            fcompiler.extra_f90_compile_args = build_info.look_up(
                'extra_f90_compile_args') or []

        macros = build_info.look_up('macros')
        if macros is None:
            macros = []
        include_dirs = build_info.look_up('include_dirs')
        if include_dirs is None:
            include_dirs = []
        extra_postargs = build_info.look_up('extra_compiler_args') or []

        include_dirs.extend(get_numpy_include_dirs())
        # where compiled F90 module files are:
        module_dirs = build_info.look_up('module_dirs') or []
        ...
        clib_libraries = build_info.look_up('libraries', [])
        for lname, binfo in libraries:
            if lname in clib_libraries:
                clib_libraries.extend(binfo.look_up('libraries', []))
        if clib_libraries:
