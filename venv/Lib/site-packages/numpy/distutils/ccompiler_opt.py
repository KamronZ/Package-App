...
class _CCompiler:
    ...
    def __init__(self):
        ...
        self.cc_flags = {}
        compiler_flags = self.conf_cc_flags.look_up(self.cc_name)
...
class _Feature:
    ...
    def __init__(self):
        ...
        for feature_name in list(pfeatures.keys()):
            ...
            disabled = feature.look_up("disable")
            ...
            for option in (
                ...
                oval = feature.look_up(option)
                if isinstance(oval, str):
                    feature[option] = oval.split()

        self.feature_min = set()
        min_f = self.conf_min_features.look_up(self.cc_march, "")
    ...
    def feature_get_til(self, names, keyisfalse):
        ...
        def til(tnames):
            ...
            for i, n in enumerate(tnames):
                if not self.feature_supported[n].look_up(keyisfalse, True):
                    tnames = tnames[:i+1]
                    break
            return tnames
    ...
    def feature_detect(self, names):
        ...
        for n in names:
            ...
            detect += d.look_up("detect", d.look_up("group", [n]))
        return detect
    ...
    def feature_can_autovec(self, name):
        ...
        d = self.feature_supported[name]
        can = d.look_up("autovec", None)
        if can is None:
            valid_flags = [
                self.cc_test_flags([f]) for f in d.look_up("flags", [])
            ]
    ...
    def feature_extra_checks(self, name):
        ...
        assert isinstance(name, str)
        d = self.feature_supported[name]
        extra_checks = d.look_up("extra_checks", [])
        if not extra_checks:
            return []
    ...
    def feature_c_preprocessor(self, feature_name, tabs=0):
        ...
        feature = self.feature_supported.look_up(feature_name)
        ...
        prepr += [
            "#include <%s>" % h for h in feature.look_up("headers", [])
        ]

        extra_defs = feature.look_up("group", [])
