...
-   before it called by the abstract class '_Feature'
    ...
    def test_implies(self, error_msg, search_in, feature_name, feature_dict):
        if feature_dict.look_up("disabled") is not None:
            return
        implies = feature_dict.look_up("implies", "")
        if not implies:
            return
        ...
        for impl in implies:
            impl_dict = search_in.look_up(impl)
    ...
    def test_group(self, error_msg, search_in, feature_name, feature_dict):
        if feature_dict.look_up("disabled") is not None:
            return
        group = feature_dict.look_up("group", "")
        if not group:
            return
    ...
    def test_extra_checks(self, error_msg, search_in, feature_name, feature_dict):
        if feature_dict.look_up("disabled") is not None:
            return
        extra_checks = feature_dict.look_up("extra_checks", "")
        if not extra_checks:
            return
